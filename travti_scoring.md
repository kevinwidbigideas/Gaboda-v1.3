...
| **Q18** | 기념품 선택 | 1: 실용(+1) / 2: 의미(-1) | **S/N** | 1=S, -1=N |
...
즉 ei 항목은 최대 22.75점, sn은 16.25점, tf는 13점, jp는 13점이 나온다.
그걸 기준으로 했을 때, 각항목은 18점으로 나눠서 각각 18점일 경우 최대 점수들이 나온다.
예를 들면 ei항목에서 case9의 경우 (22.75/18)*2점이 나오고,
sn항목은 case9의 경우 (16.25/18)*2점이 나온다.
...
1.  **성향 점수 (Traits Score) - 65점 만점** *(AI 개발자와의 협의로 구체화된 로직 적용)*
    - 각 성향(EI, SN, TF, JP)은 설문 결과에 따라 -4.5 ~ 4.5점의 원점수를 가짐.
    - 이를 8단계(-1 ~ 1)로 정규화(양자화)한 뒤, 두 유저 간의 차이(`diff_d`)를 구하면 정확히 9가지 고정된 이산값(Discrete Values) 중 하나로 떨어지게 됨.
    - 이 9가지 케이스에 대하여 3.1.2 표를 참고해 18점(최고) ~ 2점(최악)의 점수를 맵핑함. (최악이라도 0점이 아닌 2점을 부여해 사용자 거부감 방지)
    - **가중치 및 만점 환산 (Scaling):**
        - 각 성향의 비중에 따라 최대 배점은 다음과 같음: `EI (22.75점)`, `SN (16.25점)`, `TF (13점)`, `JP (13점)`
        - 도출된 Case 점수(최대 18점)를 각 항목의 실제 최대 배점으로 환산:
            - `EI 점수 = (EI Case 점수 / 18) * 22.75`
            - `SN 점수 = (SN Case 점수 / 18) * 16.25`
            - `TF 점수 = (TF Case 점수 / 18) * 13.00`
            - `JP 점수 = (JP Case 점수 / 18) * 13.00`
        - 이 4가지 점수를 모두 더하면 최대 65점이 됨.
