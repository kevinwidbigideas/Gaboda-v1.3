     ...
     <script>
        (function(){
            ...
            function initLoginState(){
                const userId = localStorage.getItem('travTRVUserId');
            ...
            function performLogin(){
                // 새 TRV-NEW-XXXX 아이디 생성
                const newNum = Math.floor(Math.random() * 10000).toString().padStart(4, '0');
                const userId = 'TRV-NEW-' + newNum;
                localStorage.setItem('travTRVUserId', userId);
                closeLoginModal();
                initLoginState();
            }

            function performLogout(){
                localStorage.removeItem('travTRVUserId');
                initLoginState();
                alert('로그아웃되었습니다.');
