...
@chemistry_bp.route('/group')
def group_page():
    return redirect(url_for('chemistry.chemistry_page'))


@chemistry_bp.route('/group-report')
def group_report_page():
    user_id = session.get('user_id')
    if not user_id:
        return redirect(url_for('auth.login_page'))
    ...
    if not travti_label or not vector_score:
        try:
            ...
            print(f"Group report DB fetch error: {e}")

    if not travti_label:
        return redirect(url_for('test.test'))

    group_id_query = request.args.get('group_id', '')
